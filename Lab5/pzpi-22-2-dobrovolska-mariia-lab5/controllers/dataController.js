...
const getRealTimeSummary = catchAsync(async (req, res, next) => {
    ...
    return res.status(200).json({
      ...
      data: {
        summary: {
          ...
          offlineDevices: 0,
          devices: [],
          platformAverageTemperature: null,
          platformAverageHumidity: null,
          platformAverageLightLevel: null,
  ...
  // Get latest data and determine online status for each device
  const deviceSummaries = await Promise.all(
    devices.map(async (device) => {
      ...
      // Device is considered online if its status in DB is 'online' AND
      // it has sent data within the last 5 minutes.
// У dataController.js, функція getRealTimeSummary, тимчасово для тесту:
const isOnline = device.status === 'online' &&
               latestData &&
               (new Date() - new Date(latestData.timestamp)) < 60 * 60 * 1000; // 60 хвилин

      return {
        deviceId: device.deviceId,
        name: device.name,
        status: isOnline ? 'online' : 'offline', // This status is now more reliable
...
const onlineCount = deviceSummaries.filter(d => d.status === 'online').length;
  const onlineDeviceIds = deviceSummaries.filter(d => d.status === 'online').map(d => d.deviceId);

  let platformAvgTemp = null;
  let platformAvgHum = null;
  let platformAvgLight = null;

  if (onlineDeviceIds.length > 0) {
    const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000); // Aggregate over the last hour
    const aggregatedStats = await SensorData.aggregate([
      {
        $match: {
          deviceId: { $in: onlineDeviceIds }, // Only for devices currently determined as online
          timestamp: { $gte: oneHourAgo }     // Data from the last hour
        }
      },
      {
        $group: {
          _id: null, // Aggregate across all matched documents
          avgTemp: { $avg: '$temperature' },
          avgHum: { $avg: '$humidity' },
          avgLight: { $avg: '$lightLevel' },
          count: { $sum: 1 } // To check if any data points were found
        }
      }
    ]);

    if (aggregatedStats.length > 0 && aggregatedStats[0].count > 0) {
      platformAvgTemp = aggregatedStats[0].avgTemp;
      platformAvgHum = aggregatedStats[0].avgHum;
      platformAvgLight = aggregatedStats[0].avgLight;
    }
  }
  ...
  res.status(200).json({
    ...
    data: {
      summary: {
        ...
        lastUpdated: new Date().toISOString(),
        devices: deviceSummaries, // Individual device statuses and latest data
        platformAverageTemperature: platformAvgTemp,
        platformAverageHumidity: platformAvgHum,
        platformAverageLightLevel: platformAvgLight
      }
    }
  });
});
module.exports = {
  ...
  getDataStatistics,
  exportData,
  getRealTimeSummary // Ensure this is exported
};