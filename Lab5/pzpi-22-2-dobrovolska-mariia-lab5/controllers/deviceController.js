...
const getUserDevices = catchAsync(async (req, res, next) => {
  const { status, search } = req.query; // Извлекаем параметры status и search из запроса
  
  // Строим объект запроса MongoDB
  const query = { 
    userId: req.user.id,
    isActive: true 
  };
  ...
  if (status) {
    query.status = status; // Добавляем фильтр по статусу, если он присутствует
  }
  
  if (search) {
    // Добавляем поиск по имени или deviceId (без учета регистра)
    query.$or = [
  ...
  const devices = await Device.find(query) // Используем построенный объект запроса
    .sort({ createdAt: -1 });
...
const registerDevice = catchAsync(async (req, res, next) => {
  ...
  // Extract userId from deviceId (format:<ctrl42>MMDD-userId-3chars)
